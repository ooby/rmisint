...
module.exports = async s => {
  ...
  const update = async () => {
    try {
      ...
      logErrors(
        await logWrap('Department ER14', () =>
          composer.syncDepartments(detailed)
        )
      )
      logErrors(
        await logWrap('Employees ER14', () => composer.syncEmployees(detailed))
      )
      logErrors(await logWrap('Rooms ER14', () => composer.syncRooms(detailed)))
      logErrors(
        await logWrap('Delete schedule ER14', () => composer.deleteSchedules())
      )
      logErrors(
        await logWrap('Add schedule ER14', () =>
          composer.syncSchedules(detailed)
        )
      )
      await cachegen
